...
class MessageProviderRegistry(providers: List[MessageProvider]) extends LazyLogging {

  private val providersByName = providers.groupBy(_.name).mapValues(_.head).map(identity)

  def buildMessage(jobName: String, messageTemplate: String, messageProviderName: Option[String]): Future[String] = {
    messageProviderName match {
      case None =>
        logger.info(s"job $jobName uses message $messageTemplate")
        Future.successful(messageTemplate)
